...
本文件只负责界面和用户交互；控制逻辑位于 ur5e_replay_fix.py。
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("UR同构遥操作 · 示教记录 · 示教回放")
    ...
    def _build_replay_group(self, parent):
        ...
        g.addWidget(self.btn_load_replay, 4, 0, 1, 2)
        g.addWidget(self.btn_start_replay, 5, 0, 1, 2)
        g.addWidget(self.btn_stop_replay, 6, 0, 1, 2)
    ...
    def start_replay_async(self):
        ...
        self.run_async(
            "启动示教回放",
            lambda: self.coordinator.start_replay(kp, kd, tool_kp, tool_kd),
        )
