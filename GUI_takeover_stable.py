...
本文件只负责界面和用户交互；控制逻辑位于 ur_takeover_stable.py。

推荐目录结构：
    GUI_takeover_stable.py
    ur_takeover_stable.py
    DMMotor.py
    Robot.py
    USBCANFD.py
...
from ur_takeover_stable import (
    ARM_DOF,
    TOTAL_MOTOR_NUM,
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("UR同构遥操作 · 示教回放 · 稳健人工接管")
    ...
    def _build_replay_group(self, parent):
        box = QGroupBox("6. UR5e 示教回放 + 主端 MIT 跟随")
        g = QGridLayout(box)
        g.setColumnStretch(1, 1)

        self.replay_kp = self._make_spin(0.0, 100.0, REPLAY_MASTER_KP_DEFAULT, 2, 1.0)
        self.replay_kd = self._make_spin(0.0, 5.0, REPLAY_MASTER_KD_DEFAULT, 2, 0.1)
        self.replay_tool_kp = self._make_spin(0.0, 100.0, REPLAY_TOOL_KP_DEFAULT, 2, 1.0)
        self.replay_tool_kd = self._make_spin(0.0, 5.0, REPLAY_TOOL_KD_DEFAULT, 2, 0.1)

        self.takeover_check = QCheckBox("允许主端人工介入并自动接管")
        ...
        note = QLabel(
            "回放：UR5e先回到轨迹起点 → 主端PV对齐 → 正式回放时UR5e和主端共享"
            "同一条记录轨迹；主端1~6轴以MIT位置跟随并叠加重力补偿，"
            "M7直接跟随记录文件中的夹爪开度。\n"
            "人工介入开启时，J1~J6与M7都会参与检测；检测器同时识别快速强推、"
            "反向运动和慢速持续拖离三类介入。确认介入后，Replay立即失去servoJ控制权，"
            "不执行普通回放的servoStop/PV保持，直接切换到绝对同构遥操作：UR5e目标关节角直接跟随主端当前DH关节角，不重新设置接管零点。"
        )
        note.setWordWrap(True)

        g.addWidget(QLabel("主端 MIT Kp基准(J1/J2)"), 0, 0)
        g.addWidget(self.replay_kp, 0, 1)
        g.addWidget(QLabel("主端 MIT Kd基准(J1/J2)"), 1, 0)
        g.addWidget(self.replay_kd, 1, 1)
        g.addWidget(QLabel("M7 MIT Kp"), 2, 0)
        g.addWidget(self.replay_tool_kp, 2, 1)
        g.addWidget(QLabel("M7 MIT Kd"), 3, 0)
        g.addWidget(self.replay_tool_kd, 3, 1)
    ...
    def start_replay_async(self):
        kp = float(self.replay_kp.value())
        kd = float(self.replay_kd.value())
        ...
        self.run_async(
            ...
            lambda: self.coordinator.start_replay(
                kp, kd, tool_kp, tool_kd, takeover_enabled
            ),
        )
