"""
不使能版：将机械臂第 6 号电机当前位置设置为新的 0 rad 零位。
...
7. 只向 ID=6 发送 DMMotor.set_zero_command。

运行示例：
    python set_motor6_zero_no_enable.py --device-index 0
    python set_motor6_zero_no_enable.py --device-index 1 --yes

注意：
- 本脚本设置的是 1~6 号机械臂中的第 6 号电机，不是第 7 个工具电机。
- 设置零位不是让电机运动到 0 rad，而是把当前机械位置定义为新的 0 rad。
- 修改 6 号电机零位会影响 q6 显示、PV 目标转换、轨迹记录/回放等。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="No-enable zero setting for arm motor 6."
    )
    parser.add_argument(
        "--device-index",
        type=int,
...
def confirm_or_exit(args: argparse.Namespace) -> None:
    ...
    print()
    print("=" * 78)
    print("不使能版：准备将机械臂第 6 号电机当前位置设置为新的 0 rad")
    ...
    print("  3. 第 6 号电机当前位置就是你希望定义的 0 rad；")
    print("  4. 你理解该操作会影响 q6 坐标系、PV 目标、记录和回放。")
    print()
...
def send_zero_no_enable(can: USBCANFD, motor_id: int, timeout_ms: float, retries: int) -> bool:
    ...
    print(f"[4] 不使能，直接向机械臂电机 {motor_id} 发送 set_zero_command...")
...
def main() -> int:
    ...
    try:
        ...
        print()
        if ok:
            pos = float(can.motors[TARGET_MOTOR_ID - 1].Position)
            if abs(pos) < 0.05:
                print("[OK] 已发送零位设置命令，且 6 号电机反馈 Position 接近 0 rad。")
            else:
                print("[WARN] 已收到 6 号电机回复，但反馈 Position 未接近 0 rad。")
